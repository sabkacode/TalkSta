    ...
    <div class="container1">
        ...
        <div class="right">
            ...
            <div class="contact">
                ...
                <?php
                    $shiva = "select follower, following from user where user_id = '$router' ";
                    $run = mysqli_query($conn, $shiva);
                    if($run){
                    $rows = $run->fetch_assoc();
                    $followerCount = $rows['follower'];
                    echo $followerCount;
                    }

                ?>
                 <div class="follows" style="display:flex;">
                    <h6>Following: <span><?php echo $followingCount; ?></span>&nbsp;&nbsp;&nbsp;&nbsp;Followers: <span><?php echo $followerCount; ?></span></h6></div>
                    <button id="followBtn" class="btn btn-success my-2 my-sm-0" data-action="<?php echo $isFollowing ? 'unfollow' : 'follow'; ?>" data-userid="<?php echo $id; ?>">
...
if ($action === 'follow') {
    $sql = "INSERT INTO followers (follow_to, follow_by) VALUES (?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ii", $target_id ,$user_id);
    if (mysqli_stmt_execute($stmt)) {
        echo json_encode(['status' => 'success', 'message' => 'Followed successfully']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Error while following']);
    }
} else if ($action === 'unfollow') {
    $sql = "DELETE FROM followers WHERE follow_to = ? AND follow_by = ?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ii", $target_id, $user_id);
    if (mysqli_stmt_execute($stmt)) {
        echo json_encode(['status' => 'success', 'message' => 'Unfollowed successfully']);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Error while unfollowing']);
    }
}
            ...
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if (response.status === 'success') {
                    if (action === 'follow') {
                        ...
                        document.getElementById('followBtn').setAttribute('data-action', 'unfollow');
                    } else {
                        document.getElementById('followBtn').innerText = 'Follow';
                        document.getElementById('followBtn').setAttribute('data-action', 'follow');
                    }
                }
            }
        };
        xhr.send('action=' + action + '&target_id=' + userId);
    });
});
</script>
